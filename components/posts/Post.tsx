...
const Post: React.FC<PostProps> = ({ post }) => {
  ...
  const handleLike = useCallback(async () => {
    ...
    const newLikesCount = liked ? optimisticLikesCount - 1 : optimisticLikesCount + 1
      ...
      <article className="rounded-xl border-2 border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 transition shadow hover:shadow-md">
        <div className="flex items-start gap-4 p-4 sm:p-6 lg:p-8">
          <Link href="/" className="block shrink-0">
            <Image src="/images/img_me.jpg" alt="Me" width={480} height={480} className="size-14 rounded object-cover" />
          </Link>
          <div className="flex-grow">
            <Body1>{post.content}</Body1>
            <div className="mt-2 sm:flex sm:items-center sm:gap-2">
              <div className="flex items-center">
                <LikeButton likesCount={optimisticLikesCount} isLiked={liked} onLike={handleLike} loading={loading} />
              </div>
            </div>
          </div>
          <Body2>
